...
- 25/09/2026 (sau nb6 + nb7): **Bước 2 Phase 2 hoàn tất — Phase 2 đóng**. (1) **Post-processing nb6 ĐẠT tiêu chí pre-registered**: chọn **G3-r1r2-r3k1** (R1 punct guard + R2 veto-delete âm tiết hợp lệ + R3 whitelist k=1) tune trên val Run 2 (+4,69 F1 val, over-corr 0,81% → 0,20%), frozen lên test: ΔF1 Run 2 **+0,93** (~40% trần lý thuyết +2,33) · over-corr **giảm** 1,88% → ≈1,46% · CleanRet +10,6 điểm → ≈96,5%; Run 1 test +0,83 (robustness PASS) → **nhận làm thành phần mặc định**. (2) **Run 3 (noise v2 + gate Telex) KHÔNG thắng Run 2 — kết quả âm có giá trị**: test F1 raw 78,68% (< 80,45%), sau re-tune post-processing riêng trên val Run 3 đạt 79,78% (vẫn < 81,38% của Run 2 + PP); calibration chỉ đạt **27,0%** non-word (không vào [23–25%], quét đủ 18 tổ hợp); **gate Telex PASS 14,6% nhưng telex_share chọn 0,0** (thêm lỗi telex làm non-word rate tăng — xung đột mục tiêu hiệu chuẩn) → lỗ hổng Telex giữ là giới hạn đã công bố. **Quyết định**: (a) cấu hình cuối đồ án = **Run 2 + post-processing — Detection F1 test ≈ 81,38% · over-correction ≈ 1,46% · Clean Retention ≈ 96,5%**; (b) **hủy Run 4 (synthetic-scaled)** — noise v2 gần đạt calibration vẫn thua Run 2, scaling cùng nguồn noise nhiều khả năng không có lợi; (c) còn mở tùy chọn theo quota: multi-pass inference cho under-editing (92% FN), full-FT ablation 1 lần; (d) ghi nhận artifact spacing của output post-processing (metric không ảnh hưởng — REPORT mục 10). Chi tiết: REPORT.md mục 13.
